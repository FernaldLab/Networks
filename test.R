...
testZeroRemoval = function() {
	...
	result4 = removeRarelyExpressedGenes(trans_and_rg_data = zero_data, max_fraction_zeroes = 0, per_group = TRUE)
	.runtest(nrow(result3$transcription_data) == 64, 'All rows with >= 25% zeroes removed')
	setVerbosity();
}
